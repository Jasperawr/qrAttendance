...
<body>
    <div class="flex justify-around h-screen w-screen bg-[url('assets/img/bg2.jpg')] bg-cover bg-center font-poppins">
        <div class="flex justify-center items-center h-full w-full bg-red-950 bg-opacity-35">    
            <div class=" flex flex-col items-center border rounded shadow pb-5 pt-[50px] px-5 bg-white">
                ...
                <form action="verify.php" method="POST" class="w-full flex flex-col gap-3" >
                    <div>
                    ...
                    <p id="errorMessage" class="opacity-0 mb-0.25 py-2 text-[11px] text-red-600 bg-red-100 rounded-md text-center w-full "></p>
    ...
    <script>
        ...
        // Example: Check if "id" exists in the URL
        if (urlParams.get("error") === "1") {
            errorMessage.classList.replace("opacity-0", "opacity-1");
            errorMessage.textContent = "Please enter your credentials";
        }
        if (urlParams.get("error") === "2") {
            console.log("ID not found in URL.");
            errorMessage.classList.replace("opacity-0", "opacity-1");
            errorMessage.textContent = "Wrong creadentials";
        }
